...
The Multi-Level Page Table (MLPT) utilizes a data structure more similar to a tree than an array. The goal of the MLPT is make each piece of a page table fit within a single page. 
...
So what if there are too many page directory entries (PDEs)? The equation to find the number of levels an MLPT needs to make a 
