...
Since we've decided our pixels are singular points, there's a problem. Smaller pictures that cover a finite amount of pixels can be abstracted too much. For example, a circle and a triangle that both cover one pixel --- the same pixel --- will appear to 