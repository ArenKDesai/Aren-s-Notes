...
### Partitioning
Spark hashes data into a partition through the equation ```hash(X) % number_of_partitions```. For example, if there are partitions 0-4 (inclusive), and your piece of data maps to 10, then 10 % 5 == 0, so the datam
