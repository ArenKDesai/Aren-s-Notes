...
Messages are idempotent if serializing the message multiple times just overwrites the data with the same message. Non-idempotent messages may cause duplicates in rare circumstances, which can be suppressed 