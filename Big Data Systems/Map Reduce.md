...
Each reduce task produces one output file. A reduce task might take multiple keys. A single reduce call takes a single key and all corresponding values. All intermediate rows with the same key go to the same reduce.r 