...
Textures are a object with properties like the texture map, mip-map, and parameters. In GLSL, this is a `sampler2D` for 2D images. A `sampler2D` must be `uniform` since it cannot change during a computation (although UV values can be varyi