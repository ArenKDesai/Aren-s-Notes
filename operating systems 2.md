...
Finally, we need to consider what happens when we delete a file. When we use a function like `rm`, we typically **unlink** a file, not fully removing it. This also means we can create more links to a file with `ln`. 