...
### Shadow Mapping
How do we compute where to place shadows? We take a picture from the light source's POV and check what objects are visible to the light source. For each pixel on an object, we need to see if the pixel is visible in the shadow map. 
