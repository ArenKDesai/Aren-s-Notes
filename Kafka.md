...
Messages are partially ordered, which means they can sometimes be compared like integers, sometimes not. If $A$ and $B$ share the same topic and key, then $B$ was produced after $A$, and we say $B$ happened after $A$. Each consumer group of the topic will consume $A$ before $B$. 
### Poll
