...
## Swapping
The kernel can organize regions of VM as segments according to how they are swapped. This improves locality as data that gets swapped to the same file all goes to the same segment. Multiple memory areas can get swapped to the same file or partiiton, but anywhere in that partition. 