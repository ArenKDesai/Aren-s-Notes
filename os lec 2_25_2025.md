...
We'll introduce the concept of a thread. These have stacks, program counters, etc. However, multiple threads share the same address space. Depending on the OS, these may run on one core or multiple cores. 
There are multiple ways to program with threads:
1. Producer / Consumer. Some nodes are producing, some are consuming. Think ROS2 or [[Kafka]]. [[big data system]]