...
We'll introduce the concept of a thread. These have stacks and stack pointers, program counters, etc. However, multiple threads share the same address space, code, heap. Depending on the OS, these may run on one core or multiple cores. 
