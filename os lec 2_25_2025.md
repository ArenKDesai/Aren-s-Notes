...
We'll introduce the concept of a thread. These have stacks, program counters, instruction pointers, heaps, . However, multiple threads share the same address space. Depending on the OS, these may run on one core or multiple cores. 
