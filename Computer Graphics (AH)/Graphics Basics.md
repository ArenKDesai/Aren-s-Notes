...
## APIs
There's plenty of APIs for graphics. Some are high-level, some low-level. For example, [[Ca]] and SVG are high-level while WebGL is low-level. Some go on top of those, like three.js. Canvas is **immediate mode API**, while SVG is a **retained mode API***. 
