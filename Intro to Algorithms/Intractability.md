...
Assume we have black-box solver for IS. We can transfer any 3SAT to IS through the clause gadget, a $k_3$ graph invented for any clause. If we have $k$ clauses, we'll have $k$ $k_3$ graphs. Out of a $k_3$ graph, we can take one node for an independent set. Thus, the max independent set is $k$. 