...
We can implement a min-cut approach to solve this. Every pixel becomes a node, and the penalty becomes the edges. We'll add a source $s$ and connect it to all nodes $i$ with capacity (penalty) $a_i$. 