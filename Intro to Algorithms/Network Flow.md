...
Input $P$ is a set of pixels (image). Separate them into two sets $A,B$, for foreground and background pixels. $a_i>=0$ is the likelihood (not probability) of $i$ being in the foreground. $bi$ is the same for background. 