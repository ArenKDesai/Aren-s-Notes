...
Schedulers are unaware of locks and unlocks, so processes can take advantage of basic spinlocks to permanently lock other processes. However, we can give locks a **ticket**, like a bakery. This is a new command called ```FetchAndAdd```. This is a **hardware primitive** that finds an old value, adds one to it, and 