...
Since none of these methods really work, we'll have the hardware work the load/store operation (since the hardware cannot be interrupted). This is done with ```xchg```, which replaces a variable's value with a value, but returns the old value. Set it to 1 in a loop, and if it returns 0 