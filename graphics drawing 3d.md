...
At this point in a traditional drawing algorithm, the pixels are drawn to the frame buffer at point $(x, y)$ with color $c$ In the Z-Buffer algorithm, the depth $z$ is introduced. If the new $z$ is less than the previous $z$, known as $pz$, then the #