...
So, we have an array of disks. How do we map blocks in the linear array to disks in the RAID? We'll denote the number of disks as $N$ and the capacity in gigabytes as $D$. There are a few options:
1. **RAID 0**: "striping". We distribute the blocks evenly. Disk 1 gets the first block, disk 2 gets the second, disk 3 gets the third, etc. This loops around when we run out of disks. If there's three disks, the fourth block goes to disk 1. This capacity is $N\times D$, and read/write performance are both $R$ time. 