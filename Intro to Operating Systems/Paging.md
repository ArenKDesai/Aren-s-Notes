...
One way to request these large pages is to request them with **mmap(MAP_HUGE)**. This will allocate a hug page or fail. The other method is by using **transparent huge pages (THP)**, which automatically uses huge