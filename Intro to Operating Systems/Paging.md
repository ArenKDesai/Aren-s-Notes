...
### Copy-On-Write
When calling ```fork```, the OS can use **copy-on-write (COW)** to create a shared mapping of the parent pages in the child address space instead of copying the page table. The child will have reduced permissions, so