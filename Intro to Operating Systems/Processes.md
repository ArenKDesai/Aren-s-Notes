...
#### fork()
fork is called to create a new process. It provides a new process ID (PID). The new process is an exact copy of the previous program (called the child to the original process as the parent), but while the parent program receives the PID of the child, the child receives the 
