...
## Address Translation
The hardware transforms virtual addresses to physical addresses. This is done with dynamic relocation, where any privlidged process can directly write to physical addresses, but unprivlidged process write to a logical address, which is added with a base register to find a physical address. However, bounds are encoded so if memory is 