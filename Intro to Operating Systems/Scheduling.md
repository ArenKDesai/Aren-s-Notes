...
CFS manages the decision on when to context switch through a control parameter **sched_latency**, which tracks the amount of time a process can run maximum before a switch. This value can be lowered when there is more processes, but no less than anot