...
NNs use a **stochastic gradient descent** (SGD) algorithm to optimize the perceptron functions. This uses calculus to determine the direction to move the parameters that would minimize the loss function. The SGD algorithm moves backwards through the perceptrons in a process known as **backwards prop
