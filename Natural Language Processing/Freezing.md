...
Prior research also shows that the earlier layers of a network extract universal features, while later layers perform task-specific modelling (2.2). This means that if a developer is fine-tuning a model for a specific task, they may want to freeze the early lay
